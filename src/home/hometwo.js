...
const Hometwo = () => {
    ...
    const handleAgeChange = (event) => {
      ...
      setAge(inputAge); // Always update the age state with the input
    
      if (/^\d+$/.test(inputAge)) {
        const ageNumber = parseInt(inputAge, 10);
    
        if (ageNumber > 18) {
          setErrorMessage('');
        } else {
          setErrorMessage('Age must be above 18.');
        }
      } else {
          ...
          <Header2/>
            ...
            <div class="banner">
                ...
                <div class="right-side">
                    <div class="card">
                        <h2>I Am...</h2>
                        <div class="leaf-container mb-5">
                            <label className="p-3">AGE:</label><br />
                            <input type="text" placeholder="enter your age" className="form-control" style={{ border: "1px solid black" }} value={age} 
                                onChange={handleAgeChange} />
                        </div>
                        {errorMessage && <div style={{ color: 'red' }}>{errorMessage}</div>}
                        <Link to="/banner_sectionweight"><button class="leafbutton">Next Question <i class="fa-solid fa-arrow-right"></i></button></Link>
                    </div>
                </div>
            </div>
